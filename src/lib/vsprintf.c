/* vsprint32f.c -- Lars Wirzenius & Linus Torvalds. */
/*
...
// 将字符数字串转换成整数，并将指针前移
static int32 skip_atoi(const int8 **s)
{
    int32 i = 0;
    while (is_digit(**s))
    {
...
// precision - 数字长度(精度)
// flags - 选项
static int8 *number(int8 *str, uint32 num, int32 base, int32 size, int32 precision, int32 flags)
{
    int8 c, sign, tmp[36];
    const int8 *digits = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    int32 i;
    int32 index;
    int8 *ptr = str;
    ...
    else// 否则如果 flags 指出是加号，则置 sign=加号，否则若类型带空格标志则 sign=空格，否则置 0
    {
...
int32 vsprintf(int8 *buf, const int8 *fmt, va_list args)
{
    ...
    int32 field_width; // 输出字段宽度
    int32 precision;   // min 整数数字个数；max 字符串中字符个数
    int32 qualifier;   // 'h', 'l' 或 'L' 用于整数字段
    ...
    repeat:
        ...
        case 'X':
        {
            str = number(str, va_arg(args, uint32), 16, field_width, precision, flags);
            break;
        }
        ...
        case 'u':
        {
            str = number(str, va_arg(args, uint32), 10, field_width, precision, flags);
            break;
        }
